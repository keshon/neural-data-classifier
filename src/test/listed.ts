...
export function testModel(
    ...
    try {
        ...
        Object.entries(testData).forEach(
            ([feature, labels]: [string, string[]]) => {
                const input: InputData = mapFeaturesToInputData(labels);
...
function mapFeaturesToInputData(features: string[]): InputData {
    return features.reduce((acc, feature) => {
        acc[feature.toLowerCase()] = 1;
        return acc;
    }, {} as InputData);
}
